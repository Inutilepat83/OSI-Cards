 ...
 */
function calculateExpectedLeft(
  columnIndex: number,
  columnWidth: number,
  gap: number,
 ...
 */
function verifyVerticalStacking(
  items: ItemPosition[],
  gap: number
  ...
  test('Each section is assigned to a specific column', async ({ page }) => {
    ...
    for (const col of columns) {
      ...
      for (const item of col.items) {
        logProgress(
          `    - Item ${item.index} (${item.sectionType}): ${item.width}x${item.height}px`
        );
      }
    }
...
test.describe('Column Positioning - Position Calculations', () => {
  ...
  test('Left positions follow column formula', async ({ page }) => {
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      logProgress(
        `  Column ${i}: expected x=${expectedLeft}px, actual x=${actualLeft}px (diff: ${diff}px)`
      );
  ...
  test('Column widths are consistent', async ({ page }) => {
    ...
    const singleSpanItems = positionsWithColumns.filter(
      (p) => p.colSpan === 1 && !['chart', 'map', 'analytics'].includes(p.sectionType)
    );
      ...
      if (col.items.length > 1) {
        for (let i = 1; i < col.items.length; i++) {
          ...
          if (diff <= 4 || actualTop >= expectedTop) {
            ...
            logProgress(
              `  Gap issue: item ${currItem.index} expected at ${expectedTop}px, got ${actualTop}px`
            );
          }
        }
  ...
  test('First item in each column starts near container top', async ({ page }) => {
    ...
    for (const col of columns) {
      if (col.items.length > 0) {
        ...
        logProgress(
          `  Column ${col.columnIndex}: first item at y=${firstItem.top}, container at y=${containerBox.y}, offset=${offsetFromContainer}px`
        );
  ...
  test('Multi-span sections have wider width', async ({ page }) => {
    ...
    for (const item of multiSpanItems) {
      const expectedMinWidth =
        singleSpanWidth * item.colSpan + GRID_CONFIG.gap * (item.colSpan - 1) - 10;

      logProgress(
        `  Multi-span item ${item.index}: width=${item.width}px, expected min=${expectedMinWidth}px`
      );
...
test.describe('Column Positioning - Summary Report', () => {
  test('Generate comprehensive position report', async ({ page }) => {
    ...
    for (const col of columns) {
      ...
      for (const item of col.items) {
        console.log(
          `    [${item.index}] ${item.sectionType.padEnd(15)} @ (${item.left}, ${item.top}) ${item.width}x${item.height}px`
        );
      }
    }
...
test.describe('Padding Tests', () => {
  ...
  test('Button padding should match spec (10px 20px)', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(count, 5); i++) {
      ...
      logProgress(
        `  Button ${i}: padding = ${styles.paddingTop} ${styles.paddingRight} ${styles.paddingBottom} ${styles.paddingLeft}`
      );
...
test.describe('Spacing Report', () => {
  test('Generate comprehensive spacing report', async ({ page }) => {
    ...
    if (verticalGaps.length > 0) {
      ...
      console.log(
        `  Vertical gaps: min=${Math.min(...verticalGaps)}px, max=${Math.max(...verticalGaps)}px, avg=${avgGap.toFixed(1)}px`
      );
    }
