...
describe('CardsState Reducer', () => {
  ...
  describe('generateCardSuccess', () => {
    it('should add card to state', () => {
      const card = CardBuilder.create().withTitle('Test Card').withId('card-1').build();
    ...
    it('should update existing card', () => {
      const card1 = CardBuilder.create().withTitle('Card 1').withId('card-1').build();

      const card2 = CardBuilder.create().withTitle('Card 1 Updated').withId('card-1').build();

      const state1 = reducer(
        initialState,
        generateCardSuccess({ card: card1, changeType: 'structural' })
      );
