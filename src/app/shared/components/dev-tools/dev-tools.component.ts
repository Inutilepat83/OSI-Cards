...
@Component({
  ...
  template: `
    <div class="dev-tools-container" *ngIf="isVisible" [class.collapsed]="isCollapsed">
      <div class="dev-tools-header" (click)="toggleCollapse()">
        ...
        <button type="button" class="toggle-btn" [attr.aria-label]="isCollapsed ? 'Expand' : 'Collapse'">
          {{ isCollapsed ? '▼' : '▲' }}
        </button>
      </div>
      ...
      <div class="dev-tools-content" *ngIf="!isCollapsed">
        ...
        <div class="dev-tools-panel" *ngIf="activeTab === 'Logs'">
          ...
          <div class="log-entries">
            ...
            >
              ...
              <span class="log-correlation" *ngIf="entry.correlationId">Corr: {{ entry.correlationId }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="dev-tools-panel" *ngIf="activeTab === 'Profiler'">
          <div class="panel-controls">
            ...
            <button type="button" (click)="clearProfilerData()" [disabled]="!isProfiling">Clear</button>
          </div>
  ...
  styles: [`
    ...
    .log-entry--debug { background: rgba(100, 100, 100, 0.1); }
    .log-entry--info { background: rgba(59, 130, 246, 0.1); }
    .log-entry--warn { background: rgba(251, 191, 36, 0.1); }
    .log-entry--error { background: rgba(239, 68, 68, 0.1); }

    .log-time { color: var(--muted-foreground, #999); }
    .log-level { font-weight: 600; text-transform: uppercase; }
    .log-context { color: var(--muted-foreground, #999); }
    .log-message { flex: 1; }
    .log-correlation { color: var(--muted-foreground, #999); font-size: 10px; }
...
export class DevToolsComponent implements OnInit, OnDestroy {
  ...
  updateProfilerData(): void {
    ...
    const maxTime = metricsArray.length > 0 
      ? Math.max(...metricsArray.map(m => m.time))
      : 1;
    ...
    this.profilerStats = [
      ...
      { label: 'Avg Time', value: metricsArray.length > 0 
