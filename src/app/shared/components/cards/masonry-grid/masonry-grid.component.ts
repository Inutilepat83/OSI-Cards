...
export class MasonryGridComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  set gap(value: number) {
    ...
    if (typeof value !== 'number' || value < 0 || !isFinite(value)) {
      this.logger.warn('Invalid gap input: expected positive number', 'MasonryGridComponent', {
        value,
      });
      this._gap = 12;
      return;
  ...
  set minColumnWidth(value: number) {
    ...
    if (typeof value !== 'number' || value <= 0 || !isFinite(value)) {
      this.logger.warn(
        'Invalid minColumnWidth input: expected positive number',
  ...
  set maxColumns(value: number) {
    ...
    if (typeof value !== 'number' || value <= 0 || !Number.isInteger(value) || !isFinite(value)) {
      this.logger.warn(
        'Invalid maxColumns input: expected positive integer',
        'MasonryGridComponent',
        { value }
      );
      this._maxColumns = 4;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['sections']) {
      ...
      if (!this.isStreaming && changes['sections'].previousValue) {
        ...
        if (
          currLength === 0 ||
          (prevLength > 0 && currLength > 0 && Math.abs(currLength - prevLength) > 3)
        ) {
  ...
  private finalizeStreamingAnimations(): void {
    ...
    if (hasChanges) {
      if (this.isDebugMode) {
        this.logger.debug(
          'Finalized streaming animations for all sections',
          'MasonryGridComponent'
        );
      }
      this.cdr.markForCheck();
  ...
  onKeydown(event: KeyboardEvent): void {
    ...
    const currentIndex = sections.findIndex(
      (s) => this.getSectionId(s.section) === currentSectionId
    );
  ...
  private computeInitialLayout(): void {
    const resolvedSections = (this.sections ?? []).filter(
      (section) =>
        section != null &&
    ...
    const validSections = resolvedSections
      .map((section) => {
        ...
        if (!section.id && !section.title && !section.type) {
          if (this.isDebugMode) {
            this.logger.warn(
              'Section filtered out - missing id, title, and type',
              'MasonryGridComponent',
              { section }
            );
          }
    ...
    const lastColumns =
      this.lastContainerWidth > 0
        ? Math.min(
            ...
            Math.max(
              1,
              Math.floor((this.lastContainerWidth + this.gap) / (this.minColumnWidth + this.gap))
            )
          )
        : 0;
    ...
    this.positionedSections = validSections.map((section, index) => {
      ...
      const isNew =
        this.isStreaming &&
        !this.previousSectionKeys.has(key) &&
        !this.animatedSectionKeys.has(key);
      ...
      if (cachedPos && !isNew && columns > 1) {
        ...
        widthExpr = columns > 1 ? `calc((100% - ${totalGap}px) / ${columns})` : '100%';
  ...
  private getSectionColSpan(section: CardSection): number {
    ...
    // title variable reserved for future use in section-type detection logic
