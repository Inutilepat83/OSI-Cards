  ...
  template: `
    <div class="error-container" *ngIf="error$ | async as error" role="alert" aria-live="assertive">
      ...
      <div class="error-actions" *ngIf="canRetry">
        <button class="error-retry" type="button" (click)="retry()">Retry</button>
      </div>
    </div>
  `,
  styles: [
...
export class ErrorDisplayComponent {
  ...
  friendlyError$: Observable<ReturnType<typeof generateUserFriendlyError>> = this.error$.pipe(
    map((error) =>
      error
        ? generateUserFriendlyError(error, 'ErrorDisplayComponent')
        : generateUserFriendlyError('', 'ErrorDisplayComponent')
    )
  );
