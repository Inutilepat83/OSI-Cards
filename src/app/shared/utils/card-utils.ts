...
export function sanitizeCardConfig(config: Partial<AICardConfig>): Partial<AICardConfig> {
  try {
    ...
    const sanitized: Partial<AICardConfig> = JSON.parse(JSON.stringify(config));
...
function sanitizeField(field: CardField): CardField {
  ...
  if (typeof field.link === 'string') {
    const sanitizedLink = SanitizationUtil.sanitizeUrl(field.link);
    if (sanitizedLink) {
  ...
  if (typeof field.email === 'string') {
    const sanitizedEmail = SanitizationUtil.sanitizeEmail(field.email);
    if (sanitizedEmail) {
...
function sanitizeAction(action: CardAction): CardAction {
  ...
  if ('url' in action && typeof action.url === 'string') {
    const sanitizedUrl = SanitizationUtil.sanitizeUrl(action.url);
    if (sanitizedUrl) {
      (sanitized as { url: string }).url = sanitizedUrl;
    }
  }
...
export function generateId(prefix: string = ID_CONSTANTS.DEFAULT_ID_PREFIX): string {
  ...
  const randomString = Math.random()
    .toString(36)
    .slice(2, 2 + ID_CONSTANTS.RANDOM_STRING_LENGTH);
