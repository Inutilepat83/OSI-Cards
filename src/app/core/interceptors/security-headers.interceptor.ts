    ...
    return next.handle(modifiedReq).pipe(
      tap((event) => {
        ...
        if (event instanceof HttpResponse) {
          ...
          const hasSecurityHeaders = Array.from(responseHeaders).some(header =>
 ...
 */
export function securityHeadersInterceptor(req: HttpRequest<unknown>, next: HttpHandler): Observable<HttpEvent<unknown>> {
