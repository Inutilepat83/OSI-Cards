/**
 * Memoization Utilities (Point 9)
 *
 * Provides decorators and utilities for caching expensive computations.
 * Supports TTL-based cache expiration and WeakMap-based caching for objects.
 *
 * @example
 * ```typescript
 * class MyService {
 ...
 * ```typescript
 * class DataService {
 *   @MemoizeAsync({ ttl: 30000 })
 *   async fetchUser(id: string): Promise<User> {
 *     return await this.http.get(`/users/${id}`).toPromise();
 *   }
 * }
 * ```
 */
export function MemoizeAsync(options: MemoizeOptions = {}): MethodDecorator {
  const { ttl = Infinity, maxSize = 100, keyGenerator = defaultKeyGenerator } = options;
 ...
 * const memoizedFn = memoize(expensiveFn, { ttl: 60000 });
 * ```
 */
export function memoize<T extends (...args: Parameters<T>) => ReturnType<T>>(
  fn: T,
  options: MemoizeOptions = {}
): T & { cache: MemoizeCache<ReturnType<T>>; clearCache: () => void } {
  const { ttl = Infinity, maxSize = 100, keyGenerator = defaultKeyGenerator } = options;
  const cache = new MemoizeCache<ReturnType<T>>(maxSize, ttl);

  const memoized = function (...args: Parameters<T>): ReturnType<T> {
    const key = keyGenerator(...(args as unknown[]));
    ...
    cache.set(key, result);
    return result;
  } as T & { cache: MemoizeCache<ReturnType<T>>; clearCache: () => void };
...
export function memoizeAsync<T extends (...args: Parameters<T>) => Promise<Awaited<ReturnType<T>>>>(
  fn: T,
  options: MemoizeOptions = {}
): T & { cache: MemoizeCache<Awaited<ReturnType<T>>>; clearCache: () => void } {
  const { ttl = Infinity, maxSize = 100, keyGenerator = defaultKeyGenerator } = options;
  const cache = new MemoizeCache<Awaited<ReturnType<T>>>(maxSize, ttl);
  ...
  } as T & { cache: MemoizeCache<Awaited<ReturnType<T>>>; clearCache: () => void };
...
/**
 * WeakMap-based memoization for object keys (Point 17)
