...
export class AgentService {
  ...
  private readonly agentApiUrl = this.config.ENV.API_URL
    ? `${this.config.ENV.API_URL}/agents`
    : '/api/agents';
  ...
  async triggerAgent(
    ...
    if (this.enableApiIntegration && agentId) {
      try {
        ...
        return (
          result || { success: false, agentId: finalAgentId, message: 'No response from agent API' }
        );
      } catch (error) {
   ...
   * @private
   */
  private triggerAgentViaApi(
    agentId: string,
    context?: Record<string, unknown>
  ): Observable<AgentTriggerResult> {
    return this.http
      .post<AgentTriggerResult>(`${this.agentApiUrl}/${agentId}/trigger`, {
        context,
  ...
  getAgentStatus(agentId: string): Observable<{ status: string; lastExecution?: Date }> {
    ...
    return this.http
      .get<{ status: string; lastExecution?: string }>(`${this.agentApiUrl}/${agentId}/status`)
      .pipe(
        map((response) => ({
  ...
  cancelAgent(executionId: string): Observable<boolean> {
    ...
    return this.http
      .post<{ cancelled: boolean }>(`${this.agentApiUrl}/cancel`, { executionId })
      .pipe(
