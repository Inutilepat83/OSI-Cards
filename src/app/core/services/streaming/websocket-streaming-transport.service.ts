...
import { Observable, Subject, BehaviorSubject, throwError, interval } from 'rxjs';
...
import { WebSocketStreamError, StreamMaxRetriesError, StreamNetworkError } from './streaming-errors';
  ...
  send(data: string | ArrayBuffer): Observable<void> {
    return new Observable(observer => {
      if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
        ...
        if (this.stateSubject.value === 'reconnecting' || this.stateSubject.value === 'connecting') {
