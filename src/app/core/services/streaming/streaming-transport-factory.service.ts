...
export class StreamingTransportFactoryService implements StreamingTransportFactory {
  ...
  create(protocol: StreamingProtocol): StreamingTransport {
    switch (protocol) {
      ...
      case 'grpc-web':
        throw new StreamProtocolError('gRPC-Web transport not yet implemented', protocol);
      ...
      default:
        throw new StreamProtocolError(`Unknown streaming protocol: ${protocol}`, protocol);
    }
  }
    ...
    return new Observable((observer) => {
      ...
      const tryNextProtocol = () => {
        if (protocolIndex >= preferredProtocols.length) {
          observer.error(
            new StreamProtocolError('All streaming protocols failed', preferredProtocols.join(', '))
          );
          return;
        }
...
export function provideStreamingTransport(_config?: {
  ...
  provide: typeof StreamingTransportFactoryService;
  useClass: typeof StreamingTransportFactoryService;
}[] {
  return [
