...
export class StreamStateError extends StreamingError {
  ...
  constructor(message: string, currentState: string, expectedState?: string, originalError?: Error) {
    super(message, originalError);
...
export class StreamingErrorFactory {
  ...
  static fromWebSocketClose(event: CloseEvent): StreamingError {
    if (event.wasClean) {
      return new StreamDisconnectedError(
        `WebSocket closed: ${event.reason || 'No reason'}`,
        true
      );
    }
