...
export class SseStreamingTransport extends StreamingTransport {
  ...
  send(data: string | ArrayBuffer): Observable<void> {
    return throwError(() => ({
      code: 'NOT_SUPPORTED',
      message:
        'SSE transport does not support sending data. Use WebSocket for bidirectional communication.',
      recoverable: false,
      timestamp: Date.now(),
    }));
  }
  ...
  isConnected(): boolean {
    return (
      this.stateSubject.value === 'connected' && this.eventSource?.readyState === EventSource.OPEN
    );
  }
