...
import { map, shareReplay, filter, takeUntil, startWith } from 'rxjs/operators';
...
export class CollaborationService {
  ...
  // Conflict detection
  private readonly conflictSubjects = new Map<string, Subject<{ conflict: boolean; message?: string }>>();

  // Generate unique user ID (simplified - would come from auth in production)
  private generateUserId(): string {
    const stored = typeof window !== 'undefined' ? localStorage.getItem('collaboration-user-id') : null;
    if (stored) {
      return stored;
    }
  ...
  getCardPresence(cardId: string): Observable<Collaborator[]> {
    ...
    return this.presenceSubjects.get(cardId)!.asObservable().pipe(
      shareReplay(1)
    );
  }
  ...
  getCardActivity(cardId: string, limit = 20): Observable<CollaborationActivity[]> {
    ...
    return this.activitySubjects.get(cardId)!.asObservable().pipe(
