...
function generateSkeletonTypes() {
  log('\n🦴 Generating Skeleton Types', colors.cyan);
  log('═'.repeat(50), colors.cyan);

  // Check if sections directory exists
  if (!fs.existsSync(SECTIONS_DIR)) {
    log(`  ✗ Sections directory not found: ${SECTIONS_DIR}`, colors.yellow);
    process.exit(1);
  }

  // Scan directory
  const sectionTypes = scanSectionsDirectory();
  log(`  📂 Scanned: ${SECTIONS_DIR}`, colors.blue);
  log(`  📊 Found ${sectionTypes.length} section types:`, colors.blue);
