    ...
    if (fs.existsSync(definitionPath)) {
      try {
        ...
        if (definition.examples?.long && !def.examples?.long) {
          ...
          def.examples.long = definition.examples.long;
        }
...
function generateDocExampleSection(type, def) {
  ...
  // Fallback chain: doc → long → demo → example → complete
...
/**
 * Generate comprehensive example section using long examples when available
 */
...
/**
 * Generate comprehensive example card with long examples
 */
function generateLongExample(sections, registry) {
  ...
  const longExampleCard = {
    ...
    sections: publicSections.map(([type, def]) => generateLongExampleSection(type, def)),
    actions: [
      {
        label: 'Generate Presentation',
        type: 'agent',
...
function generateSectionTypePortfolio(registry) {
  ...
  const chartTypes = [];
  const seenTypes = new Set(); // Track seen types to prevent duplicates

  for (const [type, def] of Object.entries(sections || {})) {
    if (def.isInternal) continue;
    if (seenTypes.has(type)) continue; // Skip if already processed
    seenTypes.add(type);
...
**ANALYTICS vs LIST DISTINCTION (CRITICAL):**
  - **Analytics section**: Use ONLY for numerical metrics, KPIs, and performance indicators
    ...
    * Value should be a measurable quantity, percentage, or short structured text credential (e.g., "Preferred SWIFT Alliance Connect partner")
    * **CRITICAL: Analytics sections must NOT contain text-only content. Each field must have a structured value (number, percentage, or short text credential) - NEVER use long descriptive paragraphs or prose text as values.**
    * NOT for descriptive facts, even if they contain numbers
    * **Examples of valid analytics values:**
      - ✅ 100 (number)
      - ✅ "47.3%" (percentage)
      - ✅ "800" (count)
      - ✅ "Preferred SWIFT Alliance Connect partner" (short credential text)
    * **Examples of INVALID analytics values (use list section instead):**
      - ❌ "Orange delivers SWIFT Alliance Connect in over 100 countries and operates connectivity for 800+ SWIFT end users worldwide" (descriptive paragraph)
      - ❌ "SWIFT connects a global network of 11,000+ financial institutions" (descriptive fact)
...
□ **For analytics sections: Each field value MUST be structured data (number, percentage, or short text credential) - NEVER long descriptive paragraphs or prose text. Analytics sections are for metrics, not descriptive content.**
...
❌ **Analytics sections with text-only content** (analytics sections MUST have structured data values - numbers, percentages, or short text credentials - NEVER long descriptive paragraphs or prose text. Use list section for descriptive content.)
