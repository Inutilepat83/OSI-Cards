...
- The default provider is `JsonCardProvider` (assets under `assets/configs/*`), but the WebSocket provider (`websocket-card-provider.service.ts`) can be swapped via `switchProvider` for live edits. Document any provider switch and whether it supports `subscribeToUpdates`.
...
3. Update `CardSection['type']` union in `src/app/models/card.model.ts` and ensure JSON templates under `assets/configs` map to the correct `type` string.
