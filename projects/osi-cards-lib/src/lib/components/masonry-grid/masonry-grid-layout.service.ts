...
import { packWithZeroGapsGuarantee } from '../../utils/zero-gap-packer.util';
...
export interface LayoutConfig {
  ...
  packingAlgorithm: 'legacy' | 'row-first' | 'skyline' | 'column-based' | 'zero-gap';
  useLegacyFallback: boolean;
  useZeroGapAlgorithm?: boolean; // Feature flag for zero-gap guarantee
  rowPackingOptions?: any;
  columnPackingOptions?: ColumnPackingOptions;
  layoutContext?: any; // LayoutContext for zero-gap algorithm
}
...
export class MasonryGridLayoutService {
  ...
  calculateLayout(
    ...
    // Zero-gap algorithm (feature flag)
    if (config.packingAlgorithm === 'zero-gap' || config.useZeroGapAlgorithm) {
      try {
        return this.calculateZeroGapLayout(sections, config, getStableKey, markAsNew);
      } catch (error) {
        if (config.useLegacyFallback) {
          ...
          if (isDevelopment) {
            console.warn('[LayoutService] Zero-gap failed, using column-based:', error);
          }
  ...
  /**
   * Calculate layout using zero-gap algorithm
   */
  private calculateZeroGapLayout(
    sections: CardSection[],
    config: LayoutConfig,
    getStableKey: (section: CardSection, index: number) => string,
    markAsNew: (section: CardSection, key: string) => boolean
    ...
    if (isDevelopment) {
      console.log('[LayoutService] 🎯 calculateZeroGapLayout called', {
        sectionsCount: sections.length,
        columns: config.columns,
        containerWidth: config.containerWidth,
      });
    ...
    // Build layout context if not provided
    const context: LayoutContext = config.layoutContext || {
      containerWidth: config.containerWidth,
      gridGap: config.gap,
      currentBreakpoint: 'lg', // Default, could be calculated from containerWidth
    ...
    if (isDevelopment) {
      console.log('[LayoutService] 📦 Zero-gap packing result', {
        positionedCount: result.positionedSections.length,
        totalHeight: result.totalHeight,
        utilization: result.utilization,
        gapCount: result.gapCount,
    ...
    // Convert to positioned sections format
    const positionedSections = result.positionedSections.map((pos, index) => {
      const section = pos.section;
      const key = getStableKey(section, pos.originalIndex);
      const isNew = markAsNew(section, key);
    ...
    if (isDevelopment) {
      console.log('[LayoutService] ✅ Zero-gap layout complete', {
        positionedSectionsCount: positionedSections.length,
        containerHeight: result.totalHeight,
        columns: config.columns,
        gapCount: result.gapCount,
  ...
  recalculateWithHeights(
    ...
    for (const item of sectionsToPlace) {
      ...
      // Note: Padding is handled at component level, expressions here don't include padding
