...
export class MasonryGridComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private computeInitialLayout(): void {
    ...
    try {
      ...
      if (this._debug) {
        console.log('[MasonryGrid] ⚡ Master engine completed in', `${calcTime.toFixed(1)}ms`);
      }

      // Convert to PositionedSection format
      this.positionedSections = layout.sections.map((section) => {
      ...
      if (this._debug) {
        console.log('[MasonryGrid] 🎉 Master Engine Layout Results:', {
          ...
          placementScore: layout.metrics.placementScore.toFixed(1),
          compacityScore: layout.metrics.compacityScore.toFixed(1),
          balanceScore: layout.metrics.balanceScore.toFixed(1),
      ...
      if (this._debug) {
        console.log('[MasonryGrid] ✅ Layout complete, isLayoutReady:', this.isLayoutReady);
      }
    } catch (error) {
